                                    ...
                                    Observable.Timer(TimeSpan.FromSeconds(5)).Subscribe(time2 =>
                                    {
                                        ...
                                        obs2 = Observable.Interval(TimeSpan.FromMilliseconds(100)).Subscribe(start3 =>
                                        {
                                            ...
                                            map.MapDesignObjects.ToList().ForEach(e =>
                                            {
                                                ...
                                                {
                                                    Character.Character chara = map.GetCharactersInRange(e.MapX, e.MapY, 0).FirstOrDefault();
                                                    if (chara != null)
                                                    {
                                                        resettap = true;
                                    ...
                                    else
                                    {
                                        switch (tm)
                                        {
                                            ...
                                            case null:
                                                ...
                                                arenaTeam.ToList().ForEach(arenauser => { SendRewards(arenauser, arenauser.ArenaTeamType == ArenaTeamType.ZENAS); });
                                            ...
                                            default:
                                                ...
                                                arenaTeam.ToList().ForEach(arenauser => { SendRewards(arenauser, arenauser.ArenaTeamType == ArenaTeamType.ERENIA); });
                                                map.Sessions.Except(arenamembers.Select(x => x.Session)).ToList().ForEach(
                                                    x =>
                                                    {
                                                        ArenaTeamMember arenauser = arenaTeam.FirstOrDefault(se => se.Session != null);
