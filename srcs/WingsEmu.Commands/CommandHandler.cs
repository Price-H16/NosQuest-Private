...
namespace WingsEmu.Commands
{
    ...
    public class CommandHandler : ICommandContainer
    {
        ...
        /// </summary>
        public CommandHandler(ILogger logger, IContainer container)
        {
            _logger = logger;
        ...
        private Task _commands_CommandExecuted(Command command, CommandResult result, ICommandContext context, IServiceProvider services)
        {
            ...
            _logger.Debug($"The command {command.Name} (from player {ctx.Player.Character.Name} [{ctx.Player.Character.CharacterId}]) has successfully been executed.");
        ...
        {
            if (!(entity is ClientSession player))
            {
                return;
            }

            var ctx = new WingsEmuIngameCommandContext(message, player, _commands);
        ...
        private Task HandleErrorAsync(IResult result, WingsEmuIngameCommandContext ctx)
        {
