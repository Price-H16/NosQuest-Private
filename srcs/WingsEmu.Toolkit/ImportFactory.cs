...
namespace WingsEmu.Toolkit
{
    public class ImportFactory
    {
        ...
        {
            string fileQuestDat = $"{_configuration.Folder}\\quest.dat";
            string fileRewardsDat = $"{_configuration.Folder}\\qstprize.dat";
            int qstCounter = 0;
            ...
            using (var questStream = new StreamReader(fileQuestDat, Encoding.GetEncoding(1252)))
            {
                ...
                DaoFactory.Instance.QuestDao.InsertOrUpdate(quests);
        ...
        {
            string fileCardDat = $"{_configuration.Folder}\\Card.dat";
            string fileCardLang = $"{_configuration.Folder}\\_code_{_configuration.Lang}_Card.txt";
            List<CardDTO> cards = new List<CardDTO>();
            Dictionary<string, string> dictionaryIdLang = new Dictionary<string, string>();
            var card = new CardDTO();
            List<BCardDTO> bcards = new List<BCardDTO>();
            DaoFactory.Instance.BCardDao.Clean();
            string line;
            int counter = 0;
                ...
                while ((line = npcIdStream.ReadLine()) != null)
                {
                    ...
                    else
                    {
                        ...
                        else if (currentLine.Length > 3 && currentLine[1] == "LAST")
                        {
                            ...
                            // investigate
                            if (DaoFactory.Instance.CardDao.LoadById(card.CardId) == null)
                            {
                                cards.Add(card);
                                counter++;
                            }
        ...
        public void ImportMapNpcs()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("mv") && o[1].Equals("2")))
            {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("eff") && o[1].Equals("2")))
            {
                if (long.Parse(currentPacket[2]) >= 20000)
                {
                    continue;
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("in") || o[0].Equals("at")))
            {
                ...
                if (DaoFactory.Instance.NpcMonsterDao.LoadByVNum(npctest.NpcVNum) == null || DaoFactory.Instance.MapNpcDao.LoadById(npctest.MapNpcId) != null || npcs.Count(i => i.MapNpcId == npctest.MapNpcId) != 0)
                {
                    continue;
                }
        ...
        public void ImportMonsters()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("mv") && o[1].Equals("3")))
            {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("in") || o[0].Equals("at")))
            {
                ...
                if (DaoFactory.Instance.NpcMonsterDao.LoadByVNum(monster.MonsterVNum) == null || DaoFactory.Instance.MapMonsterDao.LoadById(monster.MapMonsterId) != null ||
        ...
        {
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("e_info") && o[1].Equals("10")))
            {
        ...
        public void ImportNpcMonsters()
        {
            ...
            string fileNpcId = $"{_configuration.Folder}\\monster.dat";
            string fileNpcLang = $"{_configuration.Folder}\\_code_{_configuration.Lang}_monster.txt";
                ...
                while ((line = npcIdStream.ReadLine()) != null)
                {
                    ...
                    else if (currentLine.Length > 1 && currentLine[1] == "SKILL")
                    {
                        for (int i = 2; i < currentLine.Length - 3; i += 3)
                        {
                            ...
                            if (DaoFactory.Instance.SkillDao.LoadById(vnum) == null || DaoFactory.Instance.NpcMonsterSkillDao.LoadByNpcMonster(npc.NpcMonsterVNum).Count(s => s.SkillVNum == vnum) != 0)
                            {
                                continue;
                            }
                    ...
                    else if (currentLine.Length > 3 && currentLine[1] == "ITEM")
                    {
                        ...
                        for (int i = 2; i < currentLine.Length - 3; i += 3)
                        {
                            ...
                            if (DaoFactory.Instance.DropDao.LoadByMonster(npc.NpcMonsterVNum).Count(s => s.ItemVNum == vnum) != 0)
                            {
                                continue;
                            }
        ...
        public void ImportPortals()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("at") || o[0].Equals("gp")))
            {
                if (currentPacket.Length > 5 && currentPacket[0] == "at")
                {
                    map = short.Parse(currentPacket[2]);
        ...
        private static void InsertRecipe(short itemVNum, short triggerVNum, byte amount = 1, short[] recipeItems = null)
        {
            ...
            if (DaoFactory.Instance.RecipeDao.LoadByItemVNum(recipe.ItemVNum) == null)
            {
            ...
            for (int i = 0; i < recipeItems.Length; i += 2)
            {
                ...
                if (!DaoFactory.Instance.RecipeItemDao.LoadByRecipeAndItem(recipe.RecipeId, recipeItem.ItemVNum).Any())
                {
        ...
        public void ImportRecipe()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o[0].Equals("n_run") || o[0].Equals("pdtse") || o[0].Equals("m_list")))
            {
                ...
                if (currentPacket.Length > 1 && currentPacket[0] == "m_list" && (currentPacket[1] == "2" || currentPacket[1] == "4"))
                {
                    ...
                    {
                        if (DaoFactory.Instance.MapNpcDao.LoadById(mapnpcid) == null)
                        {
                            continue;
                        }
                        ...
                        if (DaoFactory.Instance.RecipeDao.LoadByNpc(mapnpcid).Any(s => s.ItemVNum == recipe.ItemVNum))
                        {
                            continue;
                        }
                ...
                if (currentPacket.Length > 1 && currentPacket[0] == "m_list" && (currentPacket[1] == "3" || currentPacket[1] == "5"))
                {
                    ...
                    {
                        RecipeDTO rec = DaoFactory.Instance.RecipeDao.LoadByNpc(mapnpcid).FirstOrDefault(s => s.ItemVNum == item);
                        if (rec != null)
                        {
                            rec.Amount = byte.Parse(currentPacket[2]);
                            DaoFactory.Instance.RecipeDao.Update(rec);
                            RecipeDTO recipedto = DaoFactory.Instance.RecipeDao.LoadByNpc(mapnpcid).FirstOrDefault(s => s.ItemVNum == item);
                            if (recipedto != null)
                            {
                                ...
                                if (!DaoFactory.Instance.RecipeItemDao.LoadByRecipeAndItem(recipeId, recipeitem.ItemVNum).Any())
                                {
                                    DaoFactory.Instance.RecipeItemDao.Insert(recipeitem);
                                }
                            }
                ...
                if (currentPacket[0].Equals("n_inv"))
                {
                    ...
                    for (int i = 5; i < currentPacket.Length; i++)
                    {
                        ...
                        if (sitem == null || shopitems.Any(s => s.ItemVNum.Equals(sitem.ItemVNum) && s.ShopId.Equals(sitem.ShopId)) ||
                            DaoFactory.Instance.ShopItemDao.LoadByShopId(sitem.ShopId).Any(s => s.ItemVNum.Equals(sitem.ItemVNum)))
                        {
                            continue;
                        }
        ...
        public void ImportShops()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o => o.Length > 6 && o[0].Equals("shop") && o[1].Equals("2")))
            {
                ...
                if (DaoFactory.Instance.ShopDao.LoadByNpc(npc.MapNpcId) == null && shops.All(s => s.MapNpcId != npc.MapNpcId))
                {
                    shops.Add(shop);
                    shopCounter++;
                }
            ...
            DaoFactory.Instance.ShopDao.Insert(shops);
                        ...
                        for (int i = 5; i < currentPacket.Length; i++)
                        {
                            ...
                            if (!currentPacket[i].Contains("."))
                            {
                                ...
                                if (shopskills.Any(s => s.SkillVNum.Equals(sskill.SkillVNum) && s.ShopId.Equals(sskill.ShopId)) ||
                                    DaoFactory.Instance.ShopSkillDao.LoadByShopId(sskill.ShopId).Any(s => s.SkillVNum.Equals(sskill.SkillVNum)))
                                {
                                    continue;
                                }
        ...
        public void ImportTeleporters()
        {
            ...
            foreach (string[] currentPacket in _configuration.Packets.Where(o =>
        ...
        {
            string fileId = $"{_configuration.Folder}\\Item.dat";
            string fileLang = $"{_configuration.Folder}\\_code_{_configuration.Lang}_Item.txt";
